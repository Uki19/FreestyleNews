...
-(instancetype)initWithFrame:(CGRect)frame{
    ...
    if(self){
        ...
        [category setShadowOffset:CGSizeMake(1, 1)];
...
//        [newsImage setContentMode:UIViewContentModeScaleAspectFit];
        ...
        gradientLayer.locations=@[@0.60,@0.70,@1.0];
...
-(void)reloadLabels{
    title.frame=CGRectMake(2, self.frame.size.height, self.frame.size.width-10, 100);
    [category sizeToFit];
    [title sizeToFit];
    title.frame=CGRectOffset(title.frame, 0, -title.frame.size.height);
    title.frame=CGRectMake(2, title.frame.origin.y, self.frame.size.width-10, title.frame.size.height);
